...
def main() -> int:
    ...
    try:
        ...
        mca_grid_min = min(m for m, _ in CONT_GRID)
        thr_grid_min = min(t for t, _ in STEP_GRID)
        sel_mca_min = min(float(r["cont_cfg"].split()[0].split("=")[1]) for r in rows)
        sel_thr_min = min(float(r["step_cfg"].split()[0].split("=")[1]) for r in rows)
        log(f"  网格下界: mca={mca_grid_min:.2f} | thr={thr_grid_min:.2f}")
        if sel_mca_min <= mca_grid_min + 1e-12:
            log("  ⚠️ 连续族选到了网格**下界的 mca** —— 网格可能没包住，外推前须补检验。")
        else:
            log("  ✓ 连续族**未贴 mca 下界**（网格已包住下侧）。"
                "边界扫描（run_drawdown_control.py）在更长网格上亦确认最优是**内部点**。")
        if sel_thr_min <= thr_grid_min + 1e-12:
            log("  ⚠️ 两态选到了网格下界的 threshold —— 同上。")
        ...
        log("    连续 `max_cut_at` 稳定在 0.02（且非网格下界，边界扫描已确认为内部最优）——")
        log("    所以**连续族比两态更适合「固定参数」**，而两态不适合每期重选。")
        log("  · 建议（较上上轮的「用连续映射」**下调**）:")
        log("    不要把回撤控制当**默认叠加层**。它产出的不是「免费的保护」，")
        log("    而是**一次风险-收益重配比**：夏普与回撤都改善，但收益近乎减半、敞口降到 1/3。")
        log("    要用就当作**独立的风险偏好选择**：用连续映射 + **固定参数**（`mca=0.02, floor=0.2`），")
        log("    并在报告里**并列**给出加/不加覆盖层的两组 headline 指标。")
        ...
        log("        未做多重检验校正。")
        log("     ③ 调杠杆成本**已计入**（`controlled_ret` 是闭合账本：仓位账 − 费用账 + 现金账）。")
        log("        因此与早期「只记仓位账」的版本相比，夏普整体更低 —— 那版高估了 0.15~0.18。")
