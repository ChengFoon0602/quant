...
def main() -> int:
    ...
    try:
        ...
        log("        未做多重检验校正。")
        log("     ③ **调杠杆成本未计入**。受控收益是按 scale 缩放净收益 → 隐含「调杠杆免费」。")
        log("        该假设的影响已在 run_drawdown_control.py 的边界扫描中量化：")
        log("        计入后夏普整体下降约 0.15~0.18（如 1.8888 → 1.7134），但**最优位置不动**")
        log("        → 抬高数值、不改变方向。上表数值应视为该假设下的上界。")
