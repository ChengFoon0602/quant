...
> **核心结论（2026-07-29 最终更新）**：在**真实 PIT 沪深 300 universe** 上，Alpha 191 价量因子库的多空策略**不成立**（LS 夏普 0.036）。搬到 **PIT 中证 500** 后信号看似翻倍（LS 1.618、WF 1.438），但经 **selection bias 消除**（因子池也进 Walk-Forward 年度重选）后，真实 WF 夏普从 1.438 降至 **0.295**（−79%），OOF IC 从 0.095 降至 **0.062**（−35%）。进一步 regime 拆分：**2015-2016 贡献几乎全部超额**（合并 SR 2.91），2017-2025 泡沫后合并 SR = **−0.15**，2021-2025 后半段 SR = **−0.27**。**信号真实存在（IC_IR 0.458）但经济价值高度 regime 依赖，多空策略在正常市不成立。** 残余价值仅在极端市场（2018 式熊市）的做空端 + 多头指数增强方向。演化全程：方法论修正 I/II → 全库重筛 → 路线② → **selection bias 修正 III**。见「修正 III」章节与 `strategies/zz500_pit_trial/report.md`。
...
8. **中证 500 多头增强落地 + 容量检验**（优先级上调）：多空路已堵死，LO 侧残余价值更高（LO 夏普 1.906 但含 selection bias——因子池全样本选的，真实 LO OOF IC 等比例下调后约 +0.04~0.05）。须叠加强制行业/市值中性化（否则只是赌小票 Beta），并用成交额加权检验中小盘流动性下的策略容量上限
