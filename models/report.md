...
6. **消除 selection bias 的终极检验**：路线② 的因子池仍是全样本 IC_IR 选出的（WF 只重训模型、不重选因子池），OOF/CV 的 IC 被高估。把因子提纯也搬进 Walk-Forward 循环（每年只用截至当年数据重选池+重训），看 WF 夏普 1.438 里还剩多少——这是判断"多少是选择偏差、多少是真 alpha"的唯一办法，也是路线② 能否落地的先决条件
7. **拆分 regime 贡献**：路线② 超额高度前置于 2015-2020（2015 单年 SR 6.29 正逢小盘泡沫），2021/2023/2025 三年为负；剔除 2015-2016 后重算，量化 edge 对小盘泡沫/特定 regime 的依赖度
8. **中证 500 指数增强落地形态 + 容量检验**：LO 超额（+54%）远强于大盘，落地方向是"中证500指数增强"而非绝对收益多空；须叠加行业/市值中性化，并用成交额加权检验中小盘流动性下的策略容量上限（1624 只 top20%≈100 只日频调仓的冲击成本可能远超 0.3% 假设）
