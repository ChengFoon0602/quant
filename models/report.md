...
2. **回撤 -18% 仍有改善空间**：动态仓位策略将回撤从 -32% 压至 -18%，但绝对水平仍偏高。
3. **未做真正样本外检验**：OOF 和组合回测都在同一历史区间内，仅靠 Purged CV 防泄露。滚动回测是下一步必须完成的关键检验。
4. **CAPM 归因使用代理市场因子**：用个股等权组合代替真实沪深300指数，beta/alpha 估计可能有偏差。
