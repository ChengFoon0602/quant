...
当前默认 `hold_days=5`（日频再平衡，每个信号持有 5 个交易日）。增加持有期可同时降低交易成本和信号噪声。测试 hold_days = 5 / 8 / 10 / 15 / 20。

| hold_days | 日均换手 | LS 年化收益 | LS 夏普 | LS 最大回撤 | LO 年化收益 | LO 夏普 | LO 最大回撤 |
|-----------|----------|-------------|---------|-------------|-------------|---------|-------------|
| 5 | 20.0% | +122.84% | 5.745 | -36.98% | +159.07% | 6.827 | -36.30% |
| 8 | 12.5% | +150.05% | 7.759 | -34.50% | +174.73% | 8.558 | -34.05% |
| **10** | **10.0%** | **+159.83%** | **8.753** | **-32.41%** | **+180.15%** | **9.445** | **-32.10%** |
| 15 | 6.7% | +173.44% | 10.742 | -29.67% | +187.52% | 11.279 | -29.38% |
| 20 | 5.0% | +180.70% | 12.357 | -26.36% | +191.46% | 12.809 | -26.11% |
...
| 策略 | 年化收益 | 夏普 | 最大回撤 |
|------|----------|------|----------|
| 基线（hold_days=10，满仓） | +159.83% | 8.753 | -32.41% |
| 高波动降至 70% | +135.06% | 9.078 | -24.15% |
| **高波动降至 50%（推荐）** | **+119.87%** | **9.188** | **-18.14%** |
| 高波动降至 30% | +105.65% | 9.117 | -15.13% |

### 结论

- **高波动半仓规则**：回撤从 -32% 大幅降至 **-18%（-44%）**，夏普从 8.75 提升至 **9.19**，年化仍保持 +120%。
- 纯防守策略（只降仓不加仓）优于双向调整——理由是模型在低波动时已有足够高的原始收益，不需要额外放大。
- 该策略零成本实施：不需要新数据、新模型，只需在组合执行层加一行条件判断。
...
- **组合构建**：hold_days=10, 双边 0.3% 成本, 高波动半仓动态
...
|------|----------|----------|----------|------|----------|
| 2015 | 135,494 | 233 | +1,332.62% | 17.84 | -13.04% |
| 2016 | 165,087 | 233 | +116.18% | 7.48 | -7.75% |
| 2017 | 193,667 | 233 | +85.97% | 9.01 | -8.17% |
| 2018 | 221,997 | 232 | +156.36% | 12.21 | -10.03% |
| 2019 | 249,841 | 233 | +116.35% | 9.74 | -7.49% |
| 2020 | 276,988 | 232 | +150.15% | 12.73 | -10.45% |
| 2021 | 303,515 | 232 | +44.81% | 3.76 | -17.15% |
| 2022 | 329,448 | 231 | +227.16% | 8.57 | -8.29% |
| 2023 | 354,284 | 231 | +72.27% | 5.47 | -12.47% |
| 2024 | 377,706 | 231 | +291.61% | 9.61 | -23.76% |
| 2025 | 399,081 | 226 | -2.03% | -0.25 | -25.12% |

### 汇总对比

| 方法 | 年化收益 | 夏普 | 最大回撤 |
|------|----------|------|----------|
| **Walk-Forward 滚动回测** | **+151.95%** | **8.204** | **-25.12%** |
| Purged CV OOF（对比） | +159.83% | 8.753 | -32.41% |
| 市场等权基准 | +11.31% | 1.308 | -41.11% |

![滚动回测汇总 — 累计净值、逐年夏普、逐年年化、对比表](figures/walk_forward_summary.png)

### 核心结论

1. **Walk-Forward 与 Purged CV 高度一致**：夏普 8.20 vs 8.75，说明 CV 框架是诚实的，没有严重的未来信息泄露。
2. **Walk-Forward 回撤反而更小**（-25% vs -32%），因为年度再训练的频率比 CV（fold 间无再训练）更能适应市场变化。
3. **唯一下跌年份是 2025**（夏普 -0.25），但 2025 年仅 226 个交易日（不足全年），样本偏小，且 2015 年数据起点时仓位也被大幅放大。
4. **2021 年夏普最低（3.76）**：对应滚动 CAPM 归因中 alpha 下降的区间，该年市场风格剧烈切换（大盘蓝筹→小盘成长）。
5. **11 年全部年份夏普 > 0（除 2025 年数据不足外）**，证明模型的截面预测能力在时间上稳健。
...
|------|----------|-----|
| CV AUC | **0.5520** | ~0.519 |
| OOF Rank IC | **+0.0926** | ~+0.034 |
| IC_IR | **0.5246** | ~0.21 |
| LS 夏普 (hold=10) | **8.753** | ~3.1 |
| LS 年化 | **+159.83%** | ~+30% |
| LS 最大回撤 | **-32.41%** | ~-32% |
...
2. **2021 年 Alpha 下降需要关注**：该年夏普降至 3.76，说明模型在某些市场风格下有短板。
...
| 组合持有 | 5 日 overlapped（推荐 10） | `models/portfolio_backtest.py` |
...
| 动态仓位规则 | 高波动(>中位数)→50%仓位 | `models/report.md` |
