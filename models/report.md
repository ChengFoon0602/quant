...
3. **多空策略在中证 500 上不成立**——消除 bias 后泡沫后时代夏普为负。
   **路线② 最终判决：条件阳性（2018 式熊市做空有效，正常市 LS 不成立），
   非此前认为的"信号全面翻倍"**
...
9. ~~**2018 式熊市做空端闭环**~~ ✅ 已完成（2026-08-12，P2，结论**否定**）——regime 过滤（指数<MA200）+ 纯空头腿（bottom 20%）把永续空头年化 −3.5% 收敛到 **+0.2%**（SR −0.11→+0.01）；但叠加真实融券成本（8%/10%）后 SR 转负至 **−0.18/−0.23**，剔除 2018 段后（borrow 8%）年化 −6.7%、SR −0.39；Block bootstrap 全部不显著（p 0.49/0.29/0.23）；MA200 慢闸门漏掉 2025-04 关税闪崩（2/21 天）。**三方向（多空/多头增强/做空端）全部否定，量价 × 中证 500 研究弧线收敛**。详见 `strategies/zz500_pit_trial/report.md` 第 5 节
