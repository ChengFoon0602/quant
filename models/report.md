...
8. ~~**中证 500 多头增强落地 + 容量检验**~~ ✅ 已完成（2026-08-12，P1，结论**否定**）——用 WF PIT-Select 去 bias 预测重跑 LO：夏普从 1.906 暴跌到 **0.25**（约 87% 是选择偏差）、相对指数超额 ≈ 0（IR −0.01）；叠加行业/市值（成交额代理）中性化后 IR 转负（**−0.56**）——信号本质是小市值/行业暴露；成交额加权冲击成本检验容量上限 < 0.5 亿（日 alpha 仅 ~1bp）。**"多头指数增强是残余价值"的判断被推翻，多空+多头全部关死，仅剩 2018 式熊市做空端**。详见 `strategies/zz500_pit_trial/report.md` 第 4 节

### 大盘方向遗留项（优先级下调）

9. **多头增强方向（沪深300）**：LO 超额跨因子池稳健（1.19/1.14 vs 0.249）——先归因 LO 超额中"top20% 等权 vs 全员等权"的结构贡献与真选股贡献
10. **factor_attribution.py 迁移 PIT**：清除其 `[:300]` 切片与 `rolling(5)` 市场平滑残留
11. **筛选链改进**：`MUST_KEEP` 改为"仅当不与更强因子冗余时保留"（曾出现 alpha055 挤掉更强的 alpha056）；因子筛选指标改用滚动窗口而非全样本 IC_IR，压制筛选期选择偏差（与第 6 项同源）
