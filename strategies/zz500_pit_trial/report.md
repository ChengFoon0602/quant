...
> alpha 显著成立、多头增强从「否定」变为「边缘显著」。** 详见文末「成本口径修正 IV」章节。
>
> **但三条红线必须同时读**：
...
> ⚠️ **成本口径修正 IV（2026-09-05）对本节的影响**：上述 P1 结论中的组合级数字（LO-raw 夏普 0.25、
> 年化 +6.4%、p=0.233）是**旧口径 0.3% 成本**算出的。收口到铁律 0.1% 后，去 bias 的 LO-raw 夏普从
> **0.25 → 0.600**（年化 +6.4% → +16.5%，p=0.233 → **0.039**），从「不显著」变为「边缘显著」。
> **但 P1 的「多头增强否定」核心依据是相对指数的 IR（−0.01）与中性化后 IR（−0.56），这两个指标
> 尚未在新口径下重算，需谨慎对待——成本口径修正会让组合绝对收益上升，可能连带改变超额与 IR 的结论。**
> 本节的数字保留旧口径原值，待新口径下重算 IR/中性化诊断后统一更新（见「成本口径修正 IV」章节）。
...
| 组合口径 | `build_portfolio` 权重追踪法，hold=5，~~双边 0.3% 成本~~（本节数字为旧口径，收口后见修正 IV），top20% 等权 |
...
### 组合对比（hold=5，~~双边 0.3%~~ → 本节为旧口径，新口径见修正 IV）
...
> ⚠️ **口径对照（修正 IV）**：LO-raw 在铁律 0.1% 成本下重算，夏普 **0.25 → 0.600**、年化 +6.4% → **+16.5%**、
> Bootstrap p 0.233 → **0.039**。绝对夏普的「否定」在收口后转为「边缘显著」；但相对指数 IR 与中性化 IR 尚未重算。
...
| 空头腿 | 截面 bottom 20% 等权做空，hold=10，~~双边 0.3% 成本~~（旧口径，收口后见修正 IV） |
...
⚠️ **「全弧线否定」被成本口径修正 IV（2026-09-05）部分推翻**：上述三连修正的关键数字
      均是在**双边 0.3% 成本（铁律 0.1% 的 3 倍）**下算出的。收口到铁律 0.1% 后：
      去 bias 的 LS 夏普 −0.188 → **+1.214**（p=0.0001，从负不显著→显著正）；
      去 bias 的 LO 夏普 0.248 → **0.600**（p=0.039，从否定→边缘显著）。
      「多空 alpha 不成立」的核心结论被反转；「多头增强否定」「做空端否定」需用新口径
      重新复核 IR/中性化/融券成本后再定论。详见「成本口径修正 IV」章节。
```
...
6. **（研究弧线收尾）** 量价 × 中证 500 三方向全部检验完毕——~~多空/多头增强/做空端均不成立~~ →
   **「多空」被成本口径修正 IV 反转（去 bias LS +1.214），「多头增强/做空端」需新口径复核**。
...
2. **「多头增强否定」需重算 IR 后定论**：去 bias LO 夏普从 0.248 → 0.600（p=0.039），绝对夏普从
   「不显著」变为「边缘显著」。但 P1 的否定核心依据是**相对指数 IR（−0.01）与中性化后 IR（−0.56）**，
   这两个指标尚未在新口径下重算——成本修正会让组合绝对收益上升，可能连带改变超额与 IR 结论，需补充检验。

3. **「做空端否定」的融券成本结论不变**：P2 的否定依据是真实融券成本（年化 8-10%），与交易成本口径
   （0.1% vs 0.3%）是**两个独立的成本**。即便交易成本收口到 0.1%，叠加 8-10% 融券成本后做空端仍大概率
   转负，该结论**不因本次修正而反转**。
