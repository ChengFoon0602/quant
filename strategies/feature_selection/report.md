...
> ⚠️ **重要更正（2026-07-27）**：本报告主体（下方各 Phase）的股票池是**事故 universe**——`sorted(cache)[:300]` 切片取出的 298 只纯深市股票，并非沪深 300（发现过程见 `models/report.md`「方法论修正 II」）。筛选链已切换到 **PIT 沪深 300**（790 只历史成员 + 逐日成员掩码）并全库重筛，结果见文末**「PIT 重筛（2026-07-27）」**章节：通过数 61 → **29**，最终池 16 → **15**（7 个新因子）。`figures/` 下三张图已被 PIT 重筛版覆盖，与主体旧表格不再对应（旧图见 git 历史）；主体保留作为旧 universe 的筛选记录。

> **核心结论（原版，事故 universe）**：对 Alpha191 全库应用三维提纯管道（IC_IR + Fama-MacBeth t + 截面有效比），从 191 个因子筛选出 **61 个通过三维标准的因子**，再经 Rank IC 相关性去重得到 **16 个低冗余因子池**，加入 2 个市场状态特征，构建最终特征矩阵 **X (1,025,288 样本 × 18 特征)**，为 LightGBM/XGBoost 非线性合成做好准备。
...
> **核心结论**：切换到 PIT 沪深 300（790 只历史成员，IC/FM/CS 全部在当日成员掩码内计算）后全库重筛：**29 个因子通过**三维过滤（旧 universe 61 个），与旧通过集仅 **13 个重叠**；冗余剔除后最终池 **15 因子 = 7 个新面孔 + 8 个旧池幸存者**。但下游 Purged CV 检验显示**新池 OOF 表现与冻结旧池相当或略差**（Rank IC 0.051 vs 0.057，WF 夏普 0.31 vs 0.43）——样本内 IC_IR 挑选存在**筛选期选择偏差**，且 Alpha 191 库在大盘股上的上限（IC≈0.05）与子集选择无关。完整对比见 `models/report.md`「全库重筛」章。

### 筛选结果对比

| 项 | 事故 universe（298 深市） | PIT CSI 300（790 历史成员） |
|----|--------------------------|------------------------------|
| \|IC_IR\| > 0.05 | 153* | 153 |
| FM \|t\| > 2.0 | — | 85 |
| CS_eff > 0.5 | — | 123 |
| **三维全过** | **61** | **29** |
| 纸老虎（IC 高但 CS_eff=0） | 23 | 23 |
| 最终池 | 16 | **15** |

*旧 universe 各单维数字见上方主体章节；两轮纸老虎名单高度重合（alpha141/105 等），该缺陷是因子公式属性，与 universe 无关。

### PIT 最终因子池（15 个）

| # | 因子 | IC_IR | FM_t | 备注 |
|---|------|-------|------|------|
| 1 | alpha087 | +0.244 | +2.94 | 新，负向平滑成交量变化 |
| 2 | alpha003 | -0.222 | -3.44 | 幸存者 |
| 3 | alpha001 | +0.215 | +3.75 | 幸存者（must_keep 基准） |
| 4 | alpha142 | +0.208 | +2.44 | 幸存者 |
| 5 | alpha011 | -0.203 | -4.93 | 幸存者 |
| 6 | alpha108 | +0.199 | +4.45 | 幸存者 |
| 7 | alpha176 | -0.187 | +2.69 | 新，日内空头压力比 |
| 8 | alpha053 | +0.179 | +3.58 | 新，2 日量价突变 |
| 9 | alpha055 | +0.175 | +2.52 | 幸存者（must_keep 避险） |
| 10 | alpha051 | +0.174 | +2.19 | 幸存者 |
| 11 | alpha018 | +0.145 | +3.20 | 新，1 日量价突变 |
| 12 | alpha043 | -0.135 | -2.97 | 新，量价变化秩相关 |
| 13 | alpha169 | -0.131 | -2.38 | 幸存者 |
| 14 | alpha120 | +0.110 | +2.57 | 新，日内振幅变化 |
| 15 | alpha072 | +0.101 | -2.95 | 新，价量秩相关 |

旧池主力 alpha068/075/110/116/144/162/166/171 在大盘股上全部落选——**因子有效性强依赖 universe，不存在"普适好因子"**。

![Rank IC 相关性矩阵（29 候选，PIT 成员掩码内）](figures/01_rank_ic_corr_heatmap.png)

![PIT 最终因子池 IC_IR](figures/02_final_pool_icir.png)

### 两个方法论备注

1. **must_keep 的代价**：alpha056（IC_IR +0.230）与强制保留的 alpha055（+0.175）相关 0.898 被剔除——规则牺牲了更强的因子。下轮应改为"仅当不与更强因子冗余时保留"。
2. **筛选期选择偏差实证**：新池样本内 IC_IR 全面高于旧池因子，但 Purged CV 的 OOF Rank IC 反而更低（0.051 vs 0.057）。全样本 IC_IR 是"过去拟合度"，不是"未来有效性"——后续筛选应改用滚动窗口指标或筛选期/验证期分离。
