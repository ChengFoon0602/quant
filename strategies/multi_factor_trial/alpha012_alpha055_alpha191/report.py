...
# 合成因子 IC
all_combos = {"等权合成": combo_equal, "ICIR加权": combo_icir}
for fid in FACTOR_IDS:
...
for name in ["alpha012", "alpha055", "alpha191", "等权合成", "ICIR加权"]:
...
plot_names = FACTOR_IDS + ["等权合成", "ICIR加权"]
...
axes.flat[5].set_visible(False)  # hide 6th subplot
...
for fid in FACTOR_IDS:
    combo_test_equal = combo_test_equal.add(test_z[fid].fillna(0), fill_value=0) / len(FACTOR_IDS)
    combo_train_equal = combo_train_equal.add(train_z[fid].fillna(0), fill_value=0) / len(FACTOR_IDS)
...
combo_test_equal = combo_test_equal / (len(FACTOR_IDS) * 2)  # fix double addition bug
combo_train_equal = combo_train_equal / (len(FACTOR_IDS) * 2)

# Recalculate combo matrices correctly
combo_test_equal = pd.DataFrame(0.0, index=close_test.index, columns=common_syms)
combo_test_icir = pd.DataFrame(0.0, index=close_test.index, columns=common_syms)
combo_train_equal = pd.DataFrame(0.0, index=close_train.index, columns=common_syms)
combo_train_icir = pd.DataFrame(0.0, index=close_train.index, columns=common_syms)
...
ax = axes[1]
names_plot = ["alpha012", "alpha055", "alpha191", "等权合成", "ICIR加权"]
...
best_combo_icir = max(combo_ic_results[n]["IC_IR"] for n in ["等权合成", "ICIR加权"])
best_ls = max(stratified_results[f]["ls_sr"] for f in FACTOR_IDS)
best_combo_ls = max(stratified_results[n]["ls_sr"] for n in ["等权合成", "ICIR加权"])
...
print(f"""
  ...
  │    但单独因子 IC 均偏弱（IC_IR < 0.11），合成后{'有' if best_combo_ls>best_ls else '无'}显著提升。       │
  │    核心问题: 三个弱信号即使独立，简单线性合成也无法                                            │
