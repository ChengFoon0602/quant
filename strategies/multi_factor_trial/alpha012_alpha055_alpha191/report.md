...
> **结论先行: 三个因子截面完全独立（max |ρ|=0.023），但所有线性合成方法均未能超越最佳单因子 alpha191（多空 SR=0.86）。正交化合成（SR=0.19）优于等权（SR=0.05），但 Bootstrap 检验 p=28%——统计不显著。独立弱信号的线性合成不可行。**
...
**三个因子截面完全独立（max |ρ| = 0.023），但所有线性合成方法均未产生统计显著的正收益。**

正交化合成（SR=0.19）确实优于等权（0.05）——剔除共线性后信息纯度提升——但离最佳单因子 alpha191（0.86）仍有巨大差距。Bootstrap 检验给了最诚实的答案：p = 28%，无法拒绝"SR ≤ 0"的零假设。

### 9.3 教训

1. **独立 ≠ 可叠加**: 因子独立只说明信息不冗余，不保证方向一致
2. **正交化有边际改善**: SR 从 0.05 → 0.19，但远远不够
3. **Bootstrap 是诚实的裁判**: 点估计看起来有提升，但统计分布说了实话
4. **线性合成的天花板**: 即使正交化，弱信号的线性组合还是弱信号

### 9.4 后续方向

1. **Sign flip alpha012**: 对 VWAP 偏离因子取反号——A 股方向与原始公式相反
2. **仅合成显著因子**: 剔除 alpha012，只合成 alpha055 + alpha191
3. **非线性合成**: LightGBM/GBDT 捕捉因子交互效应（新 CLAUDE.md 铁律 #4）
4. **PIT 数据源**: 消除幸存者偏差后再验证
5. **全量 300 只**: 提升截面统计功效
