...
（基本面池捕捉 2015 股灾前夜），但事件太少 bootstrap 不足 0.05——作为尾部预警的
置信度待更大样本验证。

## 下一步

1. （推荐）积累更长样本重测事件研究——当前 4-5 个极端事件不足以定论，扩到周度
   采样或跨指数（沪深300 对比）可增加事件数
2. （推荐）测「基本面拥挤反转」的可交易性：Q1 低拥挤 → 做多基本面因子的多空
   ——但需注意这是测度研究，转信号前先验证成本与容量
3. 归档提交
...
├── report.py                  # 全链驱动 + 对比 + 事件研究 + 4 图
├── report.md                  # 本报告
└── figures/                   # 01-04 共 4 张图
```
