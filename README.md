...
> 中期结论是 **A 股日线级截面 alpha 在扣除真实成本后不成立**，转向理解市场结构。
> **⚠️ 成本口径修正（2026-09-05）**：上述「不成立」的核心数字曾被一个交易成本 bug（代码 0.3% vs 铁律 0.1%）压低，
> 收口到铁律 0.1% 后，多空 alpha 在扣除真实成本后**显著成立**（沪深300 LS 0.036→1.175、中证500 去bias LS −0.188→+1.214）。
> 该发现已写入 models 与 zz500 报告的「方法论/成本口径修正 IV」章节，详见各报告。
