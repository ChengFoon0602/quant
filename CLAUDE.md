...
23. ⚠️ **滑点未进主口径（与 `max_turnover` 同类失效模式）** —— 已**量化**（`run_slippage_impact.py`），
    **是否改铁律 3 待你定夺**：

    机制已接入（默认 `slippage=0.0`，**不改任何历史数字**）：`build_weight_portfolio` /
    `build_portfolio` / `PortfolioOrchestrator` 三处新增 `slippage` 参数，按**总换手**计提
    （`(买入+卖出换手) × slippage`）；`CostModel.deduction(..., include_slippage=True)` 同步支持。

    **实测（真实数据，同一组合仅切换 slippage=0.0 vs 0.0005）**：

    | 链路 | 夏普（无滑点） | 夏普（万5滑点） | Δ夏普 | 相对 | Δ年化 |
    |---|---|---|---|---|---|
    | models | 1.1753 | 0.7307 | −0.4445 | **−37.8%** | −8.16pp |
    | zz500 | 2.9945 | 2.5739 | −0.4206 | **−14.0%** | −10.86pp |
    | zz500_wf | 1.2135 | 0.7382 | −0.4754 | **−39.2%** | −7.26pp |
    | fundamental | 0.4583 | 0.4082 | −0.0501 | −10.9% | −0.29pp |

    **判读**：
    - 滑点按**总换手**计提 → 换手越高的链路掉得越狠（models / zz500_wf 换手最高，掉 ~38%）。
    - **滑点（双边 0.1%）不是「与佣金等量」那么简单** —— 对高换手多空策略，
      它让夏普近乎腰斩。若纳入铁律 3，等价于「摩擦成本翻倍」，
      **会实质性削弱「多空 alpha 显著成立」的结论**（如 models LS 1.175 → 0.73）。
    - 基本面（月度、换手极低）几乎不受影响。
    - ⚠️ 该量级敏感于滑点费率假设（万 5 是日线级默认，真实盘口滑点可能更低/更高），
      且复现未含 PIT 掩码与 bias 剔除 → 只有差值有量级参考意义。

    **这是本轮最重要的发现**：之前所有「alpha 成立」的结论都建立在「无滑点」口径上，
    而滑点对高换手策略是**一阶量级**的摩擦。要不要把滑点纳入铁律 3、回改报告，
    是一个需要你拍板的方向性决策。

    **敏感性网格（2026-09-15，`run_slippage_impact.py`，滑点 0 ~ 万12）** —— 结论更细：

    | 链路 | 万0 | 万3 | 万5 | 万8 | 万12 | 跌破 0.5 于 |
    |---|---|---|---|---|---|---|
    | models | 1.1753 | 0.9087 | 0.7307 | 0.4636 | 0.1070 | 7.59 万 |
    | zz500（含 bias） | 2.9945 | 2.7423 | 2.5739 | 2.3208 | 1.9825 | 不翻转 |
    | zz500_wf（去 bias） | 1.2135 | 0.9284 | 0.7382 | 0.4528 | 0.0720 | 7.50 万 |
    | fundamental | 0.4583 | 0.4283 | 0.4082 | 0.3781 | 0.3378 | 本就 < 0.5 |

    - **归零点：全网格（≤万12）没有任何链路夏普归零** —— alpha 缩小但不消失。
    - **「多空 alpha 成立」不因滑点被推翻，但强度被显著削弱**：万5 假设下，
      去 bias 夏普从 ~1.2 掉到 ~0.73（仍正、仍显著，但不再是强 alpha）；
...
26. **无测试覆盖率度量**；`tests/cross_validate_alpha191.py` 不匹配 `test_*.py`
    ...
    **边界扫描（2026-09-15，`run_drawdown_control.py` 第三节）**：把连续族 `max_cut_at`
    ...
    | mca（最优 floor） | 夏普 | 计入调杠杆成本后 | CAGR | 平均仓位 | Σ\|Δscale\| | 调杠杆成本 |
    |---|---|---|---|---|---|---|
    | 0.002 | 1.6147 | 1.4676 | 9.92% | 0.472 | 133.29 | 0.1363 |
    | 0.005 | 1.6749 | 1.5057 | 8.13% | 0.394 | 121.26 | 0.1241 |
    | 0.008 | 1.7465 | 1.5715 | 8.89% | 0.409 | 131.27 | 0.1343 |
    | 0.012 | 1.8513 | 1.6863 | 9.87% | 0.428 | 129.11 | 0.1321 |
    | **0.020** | **1.8888** | **1.7134** | 8.60% | 0.385 | 117.35 | 0.1201 |
    | 0.030 | 1.7981 | 1.6615 | 10.85% | 0.490 | 119.45 | 0.1222 |
    | 0.050 | 1.5788 | 1.4745 | 12.13% | 0.602 | 115.69 | 0.1184 |

    1. ✅ **最优在 `mca=0.020`，是内部点** —— 下探后夏普反而下降（1.89 → 1.61）。
       **修正上一轮判断**：WF 里「每个窗口都贴下界」是**网格粒度问题**（`0.01` 与 `0.02`
       之间的有效档位），不是退化的边界偏好。**「网格没包住」的判断被证伪。**
    2. **调杠杆成本有实质影响但不改方向**：受控收益是按 `scale` 缩放净收益 → 默认假设
       **调杠杆免费**。计入后夏普整体下降 **0.15~0.18**（1.8888 → 1.7134），
       但**最优位置不动**（仍 `mca=0.020`），且仍显著高于无控制的 1.1753。
       → WF 报告的夏普数值应视为**该假设下的上界**。
    3. **Σ|Δscale| 非单调**（115.7 → 133.3，仅 1.15×）。实测与直觉相反：`mca` 极小时
       scale 退化为 `{floor, 1}` 的**双态振荡**（受控回撤一收窄就恢复满仓），
       既不是「长期贴下限」也不是「切换次数爆炸」。已在 `test_drawdown_control.py` 钉住。
